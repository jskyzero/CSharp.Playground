# .NET and C#: api overview
`jskyzero` `2018/07/26`

唔，我們已經說過.NET標準有多種實現，雖然我們會根據不同選擇使用不同的實現，但是，我們還是希望在不同實現間“遷移”是盡可能的沒有代價，那麼，我們可能需要關注下.NET標準支持的API並且使用盡可能在不同實現中都支持的那些API。

據悉，.NET標準本身是一個向前兼容不斷擴充的，當然我們選擇，盡可能低的接口可以保證盡可能高的兼容性，接下來會選擇.NET Standard 2.0為準展開敘述，為什麼是2.0呢，其一是目前MONO和Unity2018已經支持了該版本，這將是咱使用C#語言主要的地方。其二是咱一貫的作風是使用最新版本，畢竟新版本，新特性，新問題。
...
+ [.NET API Browser: .NET Standard 1.0](https://docs.microsoft.com/en-us/dotnet/api/index?view=netstandard-1.0)
+ [Updated scripting runtime in Unity 2018.1: What does the future hold?](https://blogs.unity3d.com/cn/2018/03/28/updated-scripting-runtime-in-unity-2018-1-what-does-the-future-hold/)